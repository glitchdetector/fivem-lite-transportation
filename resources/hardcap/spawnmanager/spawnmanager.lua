...
function spawnPlayer(spawnIdx, cb)
    ...
    Citizen.CreateThread(function()
        ...
        if not spawnIdx then
            spawnIdx = math.random(#spawnPoints)
            print("spawnIdx " .. spawnIdx)
        end
        ...
        if (not spawn.x) or (not spawn.y) or (not spawn.z) or (not spawn.heading) then
            print("malformed / missing spawn data")
        end
        print(("spawn %s %s %s %s"):format(spawn.x, spawn.y, spawn.z, spawn.heading))
        ...
        if spawn.model then
            print("loading model " .. spawn.model)
        ...
        -- spawn the player
        -- ResurrectNetworkPlayer(PlayerId(), spawn.x, spawn.y, spawn.z, spawn.heading)
        local ped = PlayerPedId()
